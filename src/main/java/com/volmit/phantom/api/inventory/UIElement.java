...
public class UIElement implements Element
{
	...
	public ItemStack computeItemStack()
	{
		...
		{
			D.as("Window Manager > Element").f("Failed to process item " + getMaterial().getMaterial().name());
			e.printStackTrace();
		}
	...
	public short getEffectiveDurability()
	{
		...
		else
		{
			...
			return (short) M.iclip(prog, 1, (getMaterial().getMaterial().getMaxDurability() - 1));
		}
	}
}
