...
public class Schematic
{
	 ...
	 *            the z
	 */
	public void setStripY(Brush b, int x, int z)
	{
		if(x < dimension.getWidth() && z < dimension.getDepth() && x >= 0 && z >= 0)
		{
			for(int i = 0; i < dimension.getHeight(); i++)
			{
				MaterialBlock mb = b.brush(Bukkit.getWorlds().get(0).getSpawnLocation().clone().add(x, i, z)).random();	
	 ...
	 *            the z
	 */
	public void setStripX(Brush b, int z, int y)
	{
		if(y < dimension.getHeight() && z < dimension.getDepth() && y >= 0 && z >= 0)
		{
			for(int i = 0; i < dimension.getWidth(); i++)
			{
				MaterialBlock mb = b.brush(Bukkit.getWorlds().get(0).getSpawnLocation().clone().add(i, y, z)).random();	
	 ...
	 *            the y
	 */
	public void setStripZ(Brush b, int x, int y)
	{
		if(x < dimension.getWidth() && y < dimension.getHeight() && x >= 0 && y >= 0)
		{
			for(int i = 0; i < dimension.getDepth(); i++)
			{
				MaterialBlock mb = b.brush(Bukkit.getWorlds().get(0).getSpawnLocation().clone().add(x, y, i)).random();				
				set(x, y, i, mb.getMaterial(), mb.getData());
			}
		}
	}
	
	public void setFaces(Brush brush)
	{
		for(Direction i : Direction.values())
		{
			setFace(brush, i);
	...
	public void fill(Brush b)
	{
		clear(b);
	}
	
	public void clear(Brush b)
	{
		for(int i = 0; i < dimension.getWidth(); i++)
		{
			for(int j = 0; j < dimension.getHeight(); j++)
			{
				for(int k = 0; k < dimension.getDepth(); k++)
				{
					MaterialBlock mb = b.brush(Bukkit.getWorlds().get(0).getSpawnLocation().clone().add(i, j, k)).random();
					schematic[i][j][k] = mb;
				}
			}
		}
	...
	public void setFace(Brush brush, Direction d)
	{
		if(d.equals(Direction.U))
		{
			for(int i = 0; i < mx(); i++)
			{
				for(int j = 0; j < mz(); j++)
				{
					MaterialBlock mb = brush.brush(Bukkit.getWorlds().get(0).getSpawnLocation().clone().add(i, 0, j)).random();
		...
		else if(d.equals(Direction.N))
		{
			for(int i = 0; i < mx(); i++)
			{
				for(int j = 0; j < my(); j++)
				{
					MaterialBlock mb = brush.brush(Bukkit.getWorlds().get(0).getSpawnLocation().clone().add(i, j, 0)).random();
		...
		else if(d.equals(Direction.E))
		{
			for(int i = 0; i < mz(); i++)
			{
				for(int j = 0; j < my(); j++)
				{
					MaterialBlock mb = brush.brush(Bukkit.getWorlds().get(0).getSpawnLocation().clone().add(0, i, j)).random();
