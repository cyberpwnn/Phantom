...
public class PluginAnnotationProcessor extends AbstractProcessor
{
	...
	public boolean process(Set<? extends TypeElement> annots, RoundEnvironment rEnv)
	{
		...
		for(Commands.Cmd cmd : commands)
		{
			...
			if(!cmd.desc().isEmpty())
				desc.put("description", cmd.desc());
			if(cmd.aliases().length != 0)
				desc.put("aliases", cmd.aliases());
			if(!cmd.permission().isEmpty())
				desc.put("permission", cmd.permission());
			if(!cmd.permissionMessage().isEmpty())
				desc.put("permission-message", cmd.permissionMessage());
			if(!cmd.usage().isEmpty())
				desc.put("usage", cmd.usage());
		...
		for(Permissions.Perm perm : perms)
		{
			...
			for(String p : perm.children())
				children.put(p, true);
			for(String p : perm.antichildren())
				children.put(p, false);
			if(!children.isEmpty())
